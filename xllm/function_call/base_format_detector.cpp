...
StreamingParseResult BaseFormatDetector::parse_streaming_increment(
  ...
  if (_tool_indices.empty()) {
    _tool_indices = get_tool_indices(tools);
  }
  ...
  try {
    ...
    if (obj.contains("name") && obj["name"].is_string()) {
      ...
      if (_tool_indices.find(tool_name) == _tool_indices.end()) {
    ...
    if (!current_tool_name_sent_) {
      if (current_tool_call.contains("name") &&
        ...
        if (_tool_indices.find(function_name) != _tool_indices.end()) {
    ...
    if (current_tool_id_ >= 0) {
      ...
      // Convert nlohmann::json to string map
