...
void validate_flags(const std::string& model_type) {
  ...
  if (FLAGS_enable_prefix_cache && is_linear_attention_model_type(model_type) &&
    ...
    if (FLAGS_block_size <= 0) {
      LOG(FATAL) << "linear-attention prefix cache requires block_size > 0.";
    }
    if (FLAGS_max_tokens_per_chunk_for_prefill <= 0) {
      LOG(FATAL) << "linear-attention prefix cache requires "
                    "max_tokens_per_chunk_for_prefill > 0.";
    }
    if (FLAGS_max_tokens_per_chunk_for_prefill % FLAGS_block_size != 0) {
      LOG(FATAL) << "linear-attention prefix cache saves linear-state "
                 ...
                 << ") must be a multiple of block_size (" << FLAGS_block_size
                 << ").";
    }
  }
