...
inline std::optional<FlashinferPrefillPlanInfoView> parse_plan_info_vec(
  ...
  if (v.size() != 15) return std::nullopt;
...
void debug_log_prefill_inputs(
  ...
  // Log a few times to avoid spamming.
  if (log_count.fetch_add(1) >= 40)
    return;  // Increased to debug shared attention
...
void debug_dump_pinned_schedule(const std::string& uri,
  ...
  static std::atomic<int> log_count{0};
  if (log_count.fetch_add(1) >= 5) return;
  if (!pinned_int_ws.defined() || pinned_int_ws.device().is_cuda()) return;
  ...
  try {
    ...
    auto dump_i32 = [&](const char* name, int64_t off_bytes, int64_t n) {
      ...
      if (n > nn) s << ", ...";
      s << "]";
      LOG(INFO) << "[flashinfer_plan_debug] " << uri << " " << s.str();
    };

    LOG(INFO) << "[flashinfer_plan_debug] " << uri
              << " pinned_int_ws bytes=" << pinned_bytes
              << " padded_batch_size=" << p.padded_batch_size
              << " cta_tile_q=" << p.cta_tile_q << " split_kv=" << p.split_kv;

    // Also print indptrs (host) for context
    LOG(INFO) << "[flashinfer_plan_debug] qo_indptr_host=" << qo_indptr_host;
    LOG(INFO) << "[flashinfer_plan_debug] kv_indptr_host=" << kv_indptr_host;
    ...
    LOG(WARNING) << "[flashinfer_plan_debug] failed to dump pinned schedule: "
                 << e.what();
  }
}
