...
const std::vector<std::string> kSM120ConfigNames = {
    ...
    "128x64",       // M in (16, 256], no swap
    "128x128"       // M > 256 with good wave efficiency
};

// Compute wave efficiency for a given configuration
float ComputeWaveEfficiency(int64_t m,
                            int64_t n,
                            int tile_m,
                            int tile_n,
                            int num_sms) {
  ...
  return static_cast<float>(total_tiles) / (num_waves * num_sms);
}
...
// Get configuration name based on SM120 dispatch logic
// Must match the dispatch logic in scaled_mm_sm120_fp8_dispatch.cuh
// Config naming: {TileM}x{TileN}[_swap] where TileK is always 128
// Now uses dynamic wave efficiency-based selection for M > 256
std::string GetSM120ConfigName(int64_t m,
                               int64_t n,
                               int64_t k,
                               int num_sms = 0) {
  constexpr int64_t kLargeN = 8192;
  constexpr float kMinWaveEfficiency = 0.75f;
  ...
  if (m <= 16) {
    ...
    float eff_128x64 = ComputeWaveEfficiency(m, n, 128, 64, num_sms);
    float eff_128x128 = ComputeWaveEfficiency(m, n, 128, 128, num_sms);
    if (eff_128x64 < kMinWaveEfficiency && eff_128x128 >= eff_128x64) {
      return "128x128";
    }
    return "128x64";
  } else {
    // M > 256: Dynamic selection based on wave efficiency
    float eff_128x128 = ComputeWaveEfficiency(m, n, 128, 128, num_sms);
    float eff_128x64 = ComputeWaveEfficiency(m, n, 128, 64, num_sms);

    // Prefer larger tile when efficiency is similar
    if (eff_128x128 >= eff_128x64 - kEfficiencyMargin) {
      return "128x128";
    }
    return "128x64";
  }
...
// Get TileShape description for SM120 configuration
// Must match actual config in scaled_mm_sm120_fp8_dispatch.cuh
std::string GetSM120TileShapeDesc(const std::string& config_name) {
  if (config_name == "128x32_swap") {
    return "128x32x128";  // Shape<_128, _32, _128>
  } else if (config_name == "128x64_swap" || config_name == "128x64") {
    return "128x64x128";   // Shape<_128, _64, _128>
  } else {                 // 128x128
    return "128x128x128";  // Shape<_128, _128, _128>
  }
}

// Check if configuration uses swap_ab
bool IsSwapABConfig(const std::string& config_name) {
...
BenchmarkResult RunBenchmark(int64_t m,
  ...
  result.config_name = GetSM120ConfigName(m, n, k, num_sms);

  // Compute wave efficiency based on the selected config's tile shape
  // Note: SM120 FP8 only supports TileM=128
  std::string tile_desc = GetSM120TileShapeDesc(result.config_name);
  ...
  result.wave_efficiency = ComputeWaveEfficiency(m, n, tile_m, tile_n, num_sms);
...
void AnalyzeConfigBoundaries(const std::vector<BenchmarkResult>& results,
  ...
  for (size_t i = 0; i < results.size(); ++i) {
    if (i > 0 && results[i].config_name != prev_config) {
      ...
      std::string prev_tile = GetSM120TileShapeDesc(prev_config);
      std::string curr_tile = GetSM120TileShapeDesc(results[i].config_name);
...
void GenerateSummary(
  ...
  for (const auto& [shape, results] : all_results) {
    ...
    for (const auto& [config_name, config_res] : config_results) {
      ...
      std::string tile_desc = GetSM120TileShapeDesc(config_name);
      bool swap_ab = IsSwapABConfig(config_name);
...
void GenerateRecommendations(
  ...
  for (const auto& [shape, results] : all_results) {
    ...
    for (const auto& r : results) {
      if (IsSwapABConfig(r.config_name)) {
  ...
  for (const auto& [shape, results] : all_results) {
    ...
    for (const auto& r : results) {
      file << r.m << "," << r.n << "," << r.k << "," << r.config_name << ","
           << GetSM120TileShapeDesc(r.config_name) << ","
           << (IsSwapABConfig(r.config_name) ? "true" : "false") << ","
...
void SaveAnalysisToJSON(
  ...
  for (const auto& config_name : kSM120ConfigNames) {
    ...
    file << "    \"" << config_name << "\": {\n";
    file << "      \"tile_shape\": \"" << GetSM120TileShapeDesc(config_name)
         << "\",\n";
    file << "      \"cluster_shape\": \"1x1x1\",\n";
    file << "      \"swap_ab\": "
         << (IsSwapABConfig(config_name) ? "true" : "false") << "\n";
    file << "    }";
  }
...
int main(int argc, char* argv[]) {
  ...
  for (const auto& name : kSM120ConfigNames) {
    std::cout << "  " << std::setw(15) << std::left << name
              << ": TileShape=" << std::setw(12) << GetSM120TileShapeDesc(name)
              << ", ClusterShape=1x1x1"
              << ", swap_ab=" << (IsSwapABConfig(name) ? "true" : "false")
              << std::endl;
  }
  std::cout << std::string(80, '=') << std::endl;
    ...
    for (int64_t m : m_values) {
      ...
      try {
        ...
        std::cout << "M=" << std::setw(5) << m << ", Config=" << std::setw(14)
                  ...
                  << std::setw(12) << GetSM120TileShapeDesc(result.config_name)
                  << ")"
  ...
  if (config.save_results) {
    ...
    if (ret != 0) {
      LOG(WARNING) << "Failed to create output directory: "
                   << config.output_dir;
    }
