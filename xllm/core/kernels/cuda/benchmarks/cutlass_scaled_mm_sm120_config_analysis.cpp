...
std::string GetSM120ConfigName(int64_t m, int64_t n, int64_t k) {
...
// Get TileShape description (must match actual config in dispatch.cuh)
// Format: MxNxK where Shape<_M, _N, _K>
std::string GetTileShapeDesc(const std::string& config_name) {
...
// Check if swap_ab is used
bool IsSwapAB(const std::string& config_name) {
...
ConfigResult BenchmarkWithDispatch(int64_t m,
  ...
  // Determine the configuration used
  std::string config_name = GetSM120ConfigName(m, n, k);

  return {config_name,
          GetTileShapeDesc(config_name),
          IsSwapAB(config_name),
          latency_ms,
...
TEST_F(CutlassScaledMMSM120ConfigTest, Correctness_AllConfigs) {
  ...
  for (const auto& [m, n, k, expected_config] : test_cases) {
    std::string config = GetSM120ConfigName(m, n, k);
