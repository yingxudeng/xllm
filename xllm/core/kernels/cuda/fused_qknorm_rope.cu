...
void launchFusedQKNormRope(void* qkv,
...
void fused_qk_norm_rope(
    ...
    torch::Tensor& cos_sin_cache,  // Cos/sin cache [max_position, rotary_dim]
  ...
  TORCH_CHECK(q_weight.is_cuda(), "q_weight must be a CUDA tensor");
  TORCH_CHECK(q_weight.is_contiguous(), "q_weight must be contiguous");
  TORCH_CHECK(k_weight.is_cuda(), "k_weight must be a CUDA tensor");
  TORCH_CHECK(k_weight.is_contiguous(), "k_weight must be contiguous");
  ...
  DISPATCH_HALF_TYPES(qkv.scalar_type(), "fused_qk_norm_rope_kernel", [&] {
    ...
    DISPATCH_FLOATING_TYPES(
        cos_sin_cache.scalar_type(), "fused_qk_norm_rope_kernel", [&] {
          ...
          launchFusedQKNormRope<qkv_scalar_t, cache_scalar_t>(
              qkv.data_ptr(),
