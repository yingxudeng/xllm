// Minimal standalone CUDA implementation of cache_select (in-place 2-pass).
// Kept in this directory for quick Python-side benchmarking.
...
void cache_select_cuda_launch_ptrs(
    ...
    int64_t decode_step,
    int64_t layer_num) {
  TORCH_CHECK(k_ptrs_i64.is_cuda() && v_ptrs_i64.is_cuda(),
              "k_ptrs_i64/v_ptrs_i64 must be CUDA");
  TORCH_CHECK(k_ptrs_i64.scalar_type() == torch::kInt64 &&
                  v_ptrs_i64.scalar_type() == torch::kInt64,
              "k_ptrs_i64/v_ptrs_i64 must be int64");
  TORCH_CHECK(k_ptrs_i64.is_contiguous() && v_ptrs_i64.is_contiguous(),
  ...
  TORCH_CHECK(Kv64 <= static_cast<int64_t>(UINT32_MAX),
              "Kv too large for grid.y");
  TORCH_CHECK(Layer64 <= 65535, "layer_num too large for grid.z");
...
void cache_select(const torch::Tensor& beam_index,  // [B*Beam, 1]
                  ...
                  const torch::Tensor& group_offset,  // unused
                  int64_t decode_step,
                  int64_t beam_size,
                  int64_t layer_num) {
  (void)group_offset;

  TORCH_CHECK(layer_num >= 0, "layer_num must be >= 0");
  if (layer_num == 0) {
    return;
  }
  TORCH_CHECK(static_cast<int64_t>(unshared_k_cache.size()) == layer_num,
              "unshared_k_cache length mismatch");
  TORCH_CHECK(static_cast<int64_t>(unshared_v_cache.size()) == layer_num,
              "unshared_v_cache length mismatch");

  TORCH_CHECK(beam_index.is_cuda(), "beam_index must be CUDA");
  TORCH_CHECK(block_table.is_cuda(), "block_table must be CUDA");
  TORCH_CHECK(block_table.dim() == 2 && block_table.size(1) == 1,
              "block_table must be [B, 1]");
  TORCH_CHECK(beam_index.dim() == 2 && beam_index.size(1) == 1,
              "beam_index must be [B*Beam, 1]");
  TORCH_CHECK(decode_step >= 0, "decode_step must be >= 0");
  TORCH_CHECK(beam_size > 0, "beam_size must be > 0");

  const int64_t B = block_table.size(0);
  TORCH_CHECK(beam_index.size(0) == B * beam_size,
  ...
  TORCH_CHECK(k0.is_cuda() && v0.is_cuda(), "cache must be CUDA");
  TORCH_CHECK(k0.is_contiguous() && v0.is_contiguous(),
              "cache must be contiguous");
  TORCH_CHECK(k0.dim() == 5, "cache must be 5D [MaxReq, Beam, MaxStep, Kv, D]");
  TORCH_CHECK(v0.sizes() == k0.sizes(), "k/v cache shapes must match");
  TORCH_CHECK(k0.size(1) == beam_size, "beam_size mismatch with cache");
  TORCH_CHECK(decode_step < k0.size(2),
              "decode_step must be < max_decode_step");
  ...
  for (int64_t layer = 0; layer < layer_num; ++layer) {
    ...
    TORCH_CHECK(k.is_cuda() && v.is_cuda(), "cache must be CUDA");
    TORCH_CHECK(k.is_contiguous() && v.is_contiguous(),
                "cache must be contiguous");
    TORCH_CHECK(k.sizes() == k0.sizes() && v.sizes() == k0.sizes(),
                "all layers must have same cache shape");
    TORCH_CHECK(k.scalar_type() == k0.scalar_type() &&
                    v.scalar_type() == k0.scalar_type(),
                "all layers must have same dtype");
    TORCH_CHECK(
        k.get_device() == k0.get_device() && v.get_device() == k0.get_device(),
