...
void cutlass_scaled_mm_sm75(torch::Tensor& c,
...
void cutlass_scaled_mm_sm80(torch::Tensor& c,
                            torch::Tensor const& a,
                            torch::Tensor const& b,
                            torch::Tensor const& a_scales,
                            torch::Tensor const& b_scales,
...
void cutlass_scaled_mm_sm89(torch::Tensor& c,
                            torch::Tensor const& a,
                            torch::Tensor const& b,
                            torch::Tensor const& a_scales,
                            torch::Tensor const& b_scales,
...
void cutlass_moe_mm_sm90(torch::Tensor& out_tensors,
...
void cutlass_scaled_mm_azp_sm75(torch::Tensor& c,
                                torch::Tensor const& a,
                                torch::Tensor const& b,
                                torch::Tensor const& a_scales,
                                torch::Tensor const& b_scales,
...
void cutlass_scaled_mm_azp_sm80(torch::Tensor& c,
                                torch::Tensor const& a,
                                torch::Tensor const& b,
                                torch::Tensor const& a_scales,
                                torch::Tensor const& b_scales,
...
void cutlass_scaled_mm_azp_sm89(torch::Tensor& c,
                                torch::Tensor const& a,
                                torch::Tensor const& b,
                                torch::Tensor const& a_scales,
                                torch::Tensor const& b_scales,
...
#if defined ENABLE_SCALED_MM_SM90 && ENABLE_SCALED_MM_SM90
void cutlass_scaled_mm_azp_sm90(torch::Tensor& c,
                                torch::Tensor const& a,
                                torch::Tensor const& b,
                                torch::Tensor const& a_scales,
                                torch::Tensor const& b_scales,
...
void cutlass_moe_mm(torch::Tensor& out_tensors,
  ...
  if (version_num >= 100 && version_num < 110) {
    cutlass_moe_mm_sm100(out_tensors,
                         a_tensors,
                         b_tensors,
                         a_scales,
                         b_scales,
...
#if defined ENABLE_CUTLASS_MOE_SM90 && ENABLE_CUTLASS_MOE_SM90
  if (version_num >= 90 && version_num < 100) {
    cutlass_moe_mm_sm90(out_tensors,
                        a_tensors,
                        b_tensors,
                        a_scales,
                        b_scales,
...
void get_cutlass_moe_mm_data(
  ...
  int32_t version_num = get_sm_version_num();
    ...
    (defined ENABLE_CUTLASS_MOE_SM100 && ENABLE_CUTLASS_MOE_SM100) || \
    (defined ENABLE_CUTLASS_MOE_SM120 && ENABLE_CUTLASS_MOE_SM120)
  get_cutlass_moe_mm_data_caller(topk_ids,
                                 expert_offsets,
                                 problem_sizes1,
                                 problem_sizes2,
                                 input_permutation,
...
void get_cutlass_moe_mm_problem_sizes(
  ...
  int32_t version_num = get_sm_version_num();
    ...
    (defined ENABLE_CUTLASS_MOE_SM100 && ENABLE_CUTLASS_MOE_SM100) || \
    (defined ENABLE_CUTLASS_MOE_SM120 && ENABLE_CUTLASS_MOE_SM120)
  get_cutlass_moe_mm_problem_sizes_caller(topk_ids,
                                          problem_sizes1,
                                          problem_sizes2,
                                          num_experts,
                                          n,
...
void get_cutlass_pplx_moe_mm_data(torch::Tensor& expert_offsets,
  ...
  int32_t version_num = get_sm_version_num();
    ...
    (defined ENABLE_CUTLASS_MOE_SM100 && ENABLE_CUTLASS_MOE_SM100) || \
    (defined ENABLE_CUTLASS_MOE_SM120 && ENABLE_CUTLASS_MOE_SM120)
  get_cutlass_pplx_moe_mm_data_caller(expert_offsets,
                                      problem_sizes1,
                                      problem_sizes2,
                                      expert_num_tokens,
                                      num_local_experts,
...
void cutlass_scaled_mm_azp(torch::Tensor& c,
                           torch::Tensor const& a,
                           torch::Tensor const& b,
                           torch::Tensor const& a_scales,
                           torch::Tensor const& b_scales,
