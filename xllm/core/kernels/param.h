...
struct RotaryParams {
  ...
  // Pre-formatted cos_sin cache for CUDA kernel.
  // Avoids chunk/cat operations per layer and improves performance.
  torch::Tensor cuda_cos_sin;
...
struct RmsNormStaticFp8QuantParams {
  ...
  // RMSNorm epsilon.
  double epsilon;
  // Optional output tensor. Shape: [..., hidden_size]. Dtype: float8_e4m3fn.
  // If not provided, will be allocated automatically.
  std::optional<torch::Tensor> output;
};
