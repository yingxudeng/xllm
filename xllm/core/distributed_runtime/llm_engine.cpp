...
// Resolve linear-state cache ops through the dp rank's prefix cache,
// co-locating checkpoint matching and eviction decisions in the scheduler. For
// each op:
//   - restore_src_slot_id: the checkpoint slot to copy into the live slot
//   before
//     compute (cold start only; -1 on miss or when no restore is wanted)
...
void resolve_linear_state_cache_ops(xllm::BlockManagerPool* block_manager_pool,
                                    int32_t dp_rank,
                                    xllm::RawForwardInput& input) {
  if (block_manager_pool == nullptr) {
    return;
  }
  auto* pool = block_manager_pool->linear_state_prefix_cache(dp_rank);
  if (pool == nullptr) {
    return;
  }
  for (xllm::LinearStateCacheOp& cache_op : input.linear_state_cache_ops) {
    if (!xllm::is_zero_prefix_hash(cache_op.restore_prefix_hash)) {
      cache_op.restore_src_slot_id =
          pool->match(xllm::XXH3Key(cache_op.restore_prefix_hash.data()));
    }
    if (!xllm::is_zero_prefix_hash(cache_op.save_prefix_hash)) {
      cache_op.save_dst_slot_id =
          pool->insert(xllm::XXH3Key(cache_op.save_prefix_hash.data()));
    }
  }
}
...
std::vector<RawForwardInput> LLMEngine::prepare_inputs(
  ...
  for (auto dp_rank = 0; dp_rank < dp_size_; ++dp_rank) {
    ...
    resolve_linear_state_cache_ops(
