...
LinearStateCheckpointManager::restore(
  ...
  for (size_t i = 0; i < cache_ops.size(); ++i) {
    ...
    // No restore requested: continued requests keep their warm live slot, and
    // no-prefix cold starts have nothing to copy. A non-zero restore hash with
    // no source slot means the scheduler could not resolve a checkpoint; force
    // a cold start instead of treating reused KV blocks as warm recurrent
    // state.
    if (src_slot_id < 0) {
