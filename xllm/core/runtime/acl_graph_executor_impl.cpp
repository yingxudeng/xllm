...
void debug_log_model_input_params(const ModelInputParams& params,
  ...
  LOG(INFO) << "[force_graph_eager debug] " << stage
            ...
            << ", extra_token_ids: " << params.extra_token_ids
            << ", kv_cache_tokens_nums_host: "
            << params.kv_cache_tokens_nums_host;
  params.print();
  ...
  if (params.kv_cache_tokens_nums.defined()) {
    debug_log_tensor(params.kv_cache_tokens_nums,
                     stage + " kv_cache_tokens_nums");
  }
  if (params.block_tables.defined() && params.block_tables.dim() == 2 &&
      params.block_tables.size(1) > 0) {
    debug_log_tensor(params.block_tables.select(1, 0),
                     stage + " block_tables_col0");
  }
