...
#include "core/runtime/linear_state_checkpoint_manager.h"
...
TEST_F(AclGraphExecutorTest, BatchInputCarriesLinearStateIds) {
  ...
  forward_input.input_params.has_initial_state = {0};
  forward_input = forward_input.to(*device_, torch::kFloat32);
  ...
  EXPECT_EQ(params_for_capture->num_sequences, 2);
  EXPECT_EQ(
      params_for_capture->linear_state_ids,
      std::vector<int32_t>({expected_linear_state_id, kPaddingLinearStateId}));
  ASSERT_EQ(params_for_capture->has_initial_state.size(), 2);
  EXPECT_EQ(params_for_capture->has_initial_state[0], 0);
  EXPECT_EQ(params_for_capture->has_initial_state[1], 0);
}

TEST(LinearStateCheckpointManagerTest, SaveSameSlotTagsCheckpoint) {
  std::vector<KVCache> kv_caches;
  ...
  LinearStateCheckpointManager manager(kv_caches, /*device_index=*/0);
  manager.initialize();

  LinearStatePrefixHash hash{};
  hash.fill(5);
  LinearStateCacheOp save_to_self;
  save_to_self.linear_state_id = 1;
  save_to_self.save_prefix_hash = hash;
  save_to_self.save_dst_slot_id = 1;
  manager.save({save_to_self});

  LinearStateCacheOp restore;
  restore.linear_state_id = 2;
  restore.restore_prefix_hash = hash;
  restore.restore_src_slot_id = 1;
  const std::vector<LinearStateCheckpointManager::RestoreAction> actions =
      manager.restore({restore});

  ASSERT_EQ(actions.size(), 1u);
  EXPECT_EQ(actions[0], LinearStateCheckpointManager::RestoreAction::RESTORED);
  EXPECT_TRUE(
...
TEST(LinearStateCheckpointManagerTest, RestoreMissWithHashColdStarts) {
  std::vector<KVCache> kv_caches;
  ...
  const std::vector<LinearStateCacheOp> cache_ops = {no_restore,
                                                     missing_restore};
  const std::vector<LinearStateCheckpointManager::RestoreAction> actions =
      manager.restore(cache_ops);
  ASSERT_EQ(actions.size(), 2u);
  EXPECT_EQ(actions[0], LinearStateCheckpointManager::RestoreAction::SKIPPED);
  EXPECT_EQ(actions[1],
