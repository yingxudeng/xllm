...
// lives in the scheduler's LinearStatePrefixCache. This class only performs the
...
// recurrent state currently lives in that physical slot. Scheduler-side Block
// pins keep restore sources from being evicted, while slot_tags_ verifies that
// the pinned source is physically ready. A step's inputs may be built before
// the previous step's save has physically run under schedule overlap, so
// restore degrades to a safe cold start when the tag does not match --
// recompute rather than copy stale data.
