...
std::vector<LinearStatePrefixHash> prefix_hashes_from_proto(
  ...
  for (const std::string& pb_hash : pb_hashes) {
    hashes.emplace_back(prefix_hash_from_proto(pb_hash));
  }
  return hashes;
}
...
void prefix_hashes_to_proto(
  ...
  for (const LinearStatePrefixHash& hash : hashes) {
    *pb_hashes->Add() = prefix_hash_to_proto(hash);
  }
}
...
void proto_to_forward_output(const proto::ForwardOutput& pb_output,
  ...
  raw_forward_output.linear_state_saved_prefix_hashes =
      prefix_hashes_from_proto(
          pb_output.linear_state_cache_output().saved_prefix_hashes());
...
void forward_output_to_proto(
  ...
  prefix_hashes_to_proto(linear_state_saved_prefix_hashes,
                         pb_forward_output->mutable_linear_state_cache_output()
                             ->mutable_saved_prefix_hashes());
