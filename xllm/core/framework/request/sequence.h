...
class Sequence final {
 public:
  ...
  // is drawn from the dedicated LinearStateSlotPool id space, not the generic
  // single-block pool that also backs embedding ids.
  bool has_linear_state_slot() const { return linear_state_block_.is_valid(); }
  int32_t get_linear_state_slot_id() const {
    return has_linear_state_slot() ? linear_state_block_.id() : -1;
  }
  void set_linear_state_block(Block&& linear_state_block) {
    linear_state_block_ = std::move(linear_state_block);
  }
  Block reset_linear_state_block() { return std::move(linear_state_block_); }
 ...
 private:
  ...
  // dedicated LinearStateSlotPool. Transport expands this into
  // linear_state_ids.
  Block linear_state_block_;

  // Whether linear_state_block_ holds valid recurrent state for this sequence.
