...
// NPU-specific EmbeddingVLM interface
class NPUEmbeddingVLM : public EmbeddingVLM {
 public:
  ~NPUEmbeddingVLM() override = default;
...
// NPU-specific EmbeddingVLM implementation
template <typename Model>
class NPUEmbeddingVLMImpl : public NPUEmbeddingVLM {
 public:
  NPUEmbeddingVLMImpl(Model model, const torch::TensorOptions& options)
  ...
  // Implement EmbeddingVLM interface
  ...
  void prepare_expert_weight(int32_t layer_id,
    ...
    // Most VLM embedding models don't have MoE, provide default empty
    // implementation
    return;
  }
  ...
  layer::LmHead get_lm_head() override {
    LOG(FATAL)
        << "Method 'get_lm_head' is not supported by NPU embedding VLM models. "
  ...
  void set_lm_head(layer::LmHead& head) override {
    LOG(FATAL)
        << "Method 'set_lm_head' is not supported by NPU embedding VLM models. "
