...
// NPU-specific CausalVLM interface
class NPUCausalVLM : public CausalVLM {
 public:
  ~NPUCausalVLM() override = default;
...
// NPU-specific VLM implementation
template <typename Model>
class NPUCausalVLMImpl : public NPUCausalVLM {
 public:
  NPUCausalVLMImpl(Model model, const torch::TensorOptions& options)
  ...
  // Optional: Provide default implementations for the base class's methods
  layer::LmHead get_lm_head() override {
    LOG(FATAL)
        << "Method 'get_lm_head' is not supported by NPU VLM models. Use "
  ...
  void set_lm_head(layer::LmHead& head) override {
    LOG(FATAL)
        << "Method 'set_lm_head' is not supported by NPU VLM models. Use "
           "'set_npu_lm_head' instead.";
  }

  layer::WordEmbedding get_word_embedding() override {
    LOG(FATAL) << "Method 'get_word_embedding' is not supported by NPU VLM "
                  "models. Use 'get_npu_word_embedding' instead.";
    return nullptr;  // Unreachable, but needed for compilation
  }

  void set_word_embedding(layer::WordEmbedding& embedding) override {
    LOG(FATAL) << "Method 'set_word_embedding' is not supported by NPU VLM "
