...
TEST(BatchTest, SharedMemoryRoundTripPreservesAndDefaultsLinearStateMetadata) {
  ...
  ASSERT_EQ(from_shm.input_params.linear_state_cache_ops.size(), 1u);
  EXPECT_EQ(from_shm.input_params.linear_state_cache_ops[0].linear_state_id, 4);
  EXPECT_EQ(from_shm.input_params.linear_state_cache_ops[0].request_id,
            "request-0");
  EXPECT_EQ(from_shm.input_params.linear_state_cache_ops[0].restore_prefix_hash,
            make_linear_state_prefix_hash(/*base=*/1));
  EXPECT_EQ(from_shm.input_params.linear_state_cache_ops[0].save_prefix_hash,
            make_linear_state_prefix_hash(/*base=*/33));
  ...
  EXPECT_EQ(legacy_from_shm.input_params.linear_state_ids,
            std::vector<int32_t>({-1, -1}));
  EXPECT_TRUE(legacy_from_shm.input_params.linear_state_cache_ops.empty());
  EXPECT_TRUE(
      legacy_from_shm.input_params.linear_state_evict_prefix_hashes.empty());
}

TEST(BatchTest, SharedMemoryRoundTripPreservesLinearStateOutputEvictions) {
  ...
  RawForwardOutput from_shm;
  reader_manager.raw_output_read(from_shm);
  EXPECT_EQ(from_shm.linear_state_evicted_prefix_hashes, evicted_prefix_hashes);
}
