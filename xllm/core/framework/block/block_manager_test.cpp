...
TEST(BlockManagerPoolTest, ResolveLinearStateOpsProtectsRestoresFromSaves) {
  BlockManagerPool::Options options;
  options.num_blocks(8).host_num_blocks(0).block_size(4).enable_prefix_cache(
      true);
  ...
  const int32_t restore_slot =
      prefix_cache->insert(XXH3Key(restore_hash.data()));
  ASSERT_GE(restore_slot, 1);
  ASSERT_GE(prefix_cache->insert(XXH3Key(evictable_hash.data())), 1);
...
TEST(BlockManagerPoolTest,
     ResolveLinearStateOpsSkipsSaveWhenRestoresProtected) {
  BlockManagerPool::Options options;
  options.num_blocks(8).host_num_blocks(0).block_size(4).enable_prefix_cache(
      true);
  options.single_block_capacity(4)
  ...
  const int32_t restore_slot1 =
      prefix_cache->insert(XXH3Key(restore_hash1.data()));
  const int32_t restore_slot2 =
      prefix_cache->insert(XXH3Key(restore_hash2.data()));
