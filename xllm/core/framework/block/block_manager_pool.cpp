...
void BlockManagerPool::check_dp_rank(int32_t dp_rank) const {
  CHECK_GE(dp_rank, 0);
...
void BlockManagerPool::set_linear_state_flags(
  ...
  check_dp_rank(dp_rank);
  auto& checkpoint_hashes = linear_state_checkpoint_hashes_[dp_rank];
  for (const XXH3Key& hash : prefix_hashes) {
    checkpoint_hashes.insert(hash);
  }
}
...
bool BlockManagerPool::has_linear_state(int32_t dp_rank,
  ...
  check_dp_rank(dp_rank);
  return linear_state_checkpoint_hashes_[dp_rank].find(prefix_hash) !=
         linear_state_checkpoint_hashes_[dp_rank].end();
}
...
std::vector<PrefixHash> BlockManagerPool::drain_linear_state_evictions() {
  ...
  for (size_t dp_rank = 0; dp_rank < block_managers_.size(); ++dp_rank) {
    auto& checkpoint_hashes = linear_state_checkpoint_hashes_[dp_rank];
    for (const XXH3Key& key :
         block_managers_[dp_rank]->drain_evicted_prefix_hashes()) {
      auto checkpoint_it = checkpoint_hashes.find(key);
      if (checkpoint_it == checkpoint_hashes.end()) {
        continue;
      }
      result.emplace_back(to_prefix_hash(key));
      checkpoint_hashes.erase(checkpoint_it);
    }
  }
  return result;
}
...
size_t BlockManagerPool::get_linear_state_prefix_match_blocks(
  ...
  for (size_t block_idx = safe_blocks; block_idx < shared_blocks.size();
    ...
    if (has_linear_state(dp_rank, prefix_hash)) {
