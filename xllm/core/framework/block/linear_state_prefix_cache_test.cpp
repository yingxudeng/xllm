...
TEST(LinearStatePrefixCacheTest, InsertMatchAndDedup) {
  ...
  const int32_t slot = cache.insert(h1);
  EXPECT_GE(slot, 1);
  EXPECT_TRUE(cache.contains(h1));
  EXPECT_EQ(cache.match(h1), slot);

  // Re-inserting the same hash is idempotent.
  EXPECT_EQ(cache.insert(h1), slot);

  // Unknown hashes miss.
  EXPECT_FALSE(cache.contains(make_hash(2)));
  EXPECT_EQ(cache.match(make_hash(2)), -1);
}
...
TEST(LinearStatePrefixCacheTest, AcquireLiveSlotReclaimsCheckpoint) {
  ...
  const int32_t checkpoint_slot = cache.insert(h1);
...
TEST(LinearStatePrefixCacheTest, InsertCannotReclaimLiveSlots) {
  ...
  EXPECT_EQ(cache.insert(make_hash(1)), -1);
  EXPECT_FALSE(cache.contains(make_hash(1)));
}

TEST(LinearStatePrefixCacheTest, InsertSkipsProtectedCheckpointSlots) {
  LinearStatePrefixCache cache(/*num_slots=*/3);
  const XXH3Key h1 = make_hash(1);
  const XXH3Key h2 = make_hash(2);
  const XXH3Key h3 = make_hash(3);

  const int32_t protected_slot = cache.insert(h1);
  ASSERT_GE(protected_slot, 1);
  ASSERT_GE(cache.insert(h2), 1);

  const std::unordered_set<int32_t> protected_slots = {protected_slot};
  EXPECT_GE(cache.insert(h3, protected_slots), 1);
  EXPECT_TRUE(cache.contains(h1));
  EXPECT_FALSE(cache.contains(h2));
  EXPECT_TRUE(cache.contains(h3));
}

TEST(LinearStatePrefixCacheTest, InsertFailsWhenAllCheckpointsProtected) {
  LinearStatePrefixCache cache(/*num_slots=*/3);
  const XXH3Key h1 = make_hash(1);
  const XXH3Key h2 = make_hash(2);
  const XXH3Key h3 = make_hash(3);

  const int32_t slot1 = cache.insert(h1);
  const int32_t slot2 = cache.insert(h2);
  ASSERT_GE(slot1, 1);
  ASSERT_GE(slot2, 1);

  const std::unordered_set<int32_t> protected_slots = {slot1, slot2};
  EXPECT_EQ(cache.insert(h3, protected_slots), -1);
  EXPECT_TRUE(cache.contains(h1));
  EXPECT_TRUE(cache.contains(h2));
  EXPECT_FALSE(cache.contains(h3));
}

TEST(LinearStatePrefixCacheTest, InsertExistingHashIgnoresProtectedSlots) {
  LinearStatePrefixCache cache(/*num_slots=*/2);
  const XXH3Key h1 = make_hash(1);
  ...
  const std::unordered_set<int32_t> protected_slots = {slot};
  EXPECT_EQ(cache.insert(h1, protected_slots), slot);
  EXPECT_TRUE(cache.contains(h1));
}
