...
size_t LinearStateBlockManager::compute_safe_shared_prefix_length(
  ...
  if (chunk_stride_ <= 0 || chunk_stride_ % kv_block_size_ != 0) {
    return total_shared_blocks;
  }
  const size_t block_size = static_cast<size_t>(kv_block_size_);
  const size_t chunk_stride = static_cast<size_t>(chunk_stride_);
  ...
  const size_t hash_chunks = std::min(reusable_chunks, token_chunks);
  const std::vector<PrefixHash> hashes = compute_linear_state_prefix_hashes(
      sequence->tokens(), chunk_stride, hash_chunks * chunk_stride);
  for (size_t chunk_idx = safe_shared_blocks / blocks_per_chunk;
       chunk_idx < hashes.size();
       ++chunk_idx) {
    if (prefix_cache_->contains(XXH3Key(hashes[chunk_idx].data()))) {
