...
torch::Tensor run_chunked_prefill_attention(const torch::Tensor& query,
  ...
  for (int64_t batch_idx = 0; batch_idx < batch_size; ++batch_idx) {
    ...
    int64_t num_blocks = (total_seq_len + block_size - 1) / block_size;
    auto block_row = block_table[batch_idx].slice(/*dim=*/0, 0, num_blocks);
    auto gathered_k =
        gather_kv_from_paged_cache(k_cache, block_row, total_seq_len);
    auto gathered_v =
        gather_kv_from_paged_cache(v_cache, block_row, total_seq_len);
    if (kv_repeat > 1) {
