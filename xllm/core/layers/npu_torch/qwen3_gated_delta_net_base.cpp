...
torch::Tensor Qwen3GatedDeltaNetBaseImpl::forward(
  ...
  if (attn_metadata.is_prefill) {
    ...
    xllm::kernel::CausalConv1dUpdateV2Params conv1d_v2_params;
    conv1d_v2_params.x = mixed_qkv.reshape({-1, mixed_qkv.size(-1)});
    conv1d_v2_params.conv_state = conv_cache;
    conv1d_v2_params.weight = conv_weight;
    conv1d_v2_params.conv_state_indices = state_indices;
    conv1d_v2_params.block_idx_last_scheduled_token =
        std::optional<torch::Tensor>();
    conv1d_v2_params.initial_state_idx = std::optional<torch::Tensor>();
    conv1d_v2_params.query_start_loc = attn_metadata.q_cu_seq_lens;
    conv1d_v2_params.max_query_len = attn_metadata.max_query_len;
    mixed_qkv = xllm::kernel::causal_conv1d_update_v2(conv1d_v2_params);
