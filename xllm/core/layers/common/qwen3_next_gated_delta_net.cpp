...
torch::Tensor Qwen3NextGatedDeltaNetImpl::forward(
  ...
  if (attn_metadata.is_prefill) {
    ...
    if (is_qwen3_5_model_) {
      // Keep decode conv update numerically/layout aligned with prefill.
      // This avoids layout ambiguity in backend kernels for Qwen3.5 path.
      auto conv_state = torch::index_select(conv_cache, 0, state_indices)
                            .to(mixed_qkv.dtype())
                            .contiguous();
      torch::Tensor bias;
      auto grouped_weight = conv_weight.unsqueeze(1).to(device);
      auto conv_out = torch::empty_like(mixed_qkv);

      for (int64_t t = 0; t < seq_len; ++t) {
        auto x_t = mixed_qkv.narrow(2, t, 1);
        auto window = torch::cat({conv_state, x_t}, /*dim=*/2);
        auto y_t =
            torch::conv1d(window,
                          grouped_weight,
                          bias,
                          ...
                          /*dilation=*/std::vector<int64_t>{1},
                          /*groups=*/static_cast<int64_t>(window.size(1)));
        conv_out.narrow(2, t, 1).copy_(torch::silu(y_t));
        conv_state = window.narrow(2, 1, conv_kernel_size_ - 1).contiguous();
      }
  ...
  if (attn_metadata.is_prefill) {
    ...
    if (is_qwen3_5_model_) {
      // Keep decode gating numerically aligned with prefill for Qwen3.5.
      beta = torch::sigmoid(b);
      torch::Tensor A_log_exp = A_log_.exp();
      torch::Tensor a_float = a.to(torch::kFloat32);
      torch::Tensor a_plus_dt = a_float + dt_bias_;
