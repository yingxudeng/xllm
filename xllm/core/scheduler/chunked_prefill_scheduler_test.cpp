...
TEST(ChunkedPrefillSchedulerTest,
     LinearStatePrefillKeepsChunkBudgetAtBlockBoundary) {
  ScopedValue<bool> prefix_cache_guard(&FLAGS_enable_prefix_cache, true);
  ...
  auto requests =
      generate_request({64}, {10}, std::nullopt, std::nullopt, 30000);
  auto* sequence = requests[0]->sequences()[0].get();

  EXPECT_EQ(scheduler->get_linear_state_safe_prefill_tokens(
                sequence, /*token_budget=*/16),
            16u);
}

TEST(ChunkedPrefillSchedulerTest,
     LinearStatePrefillRoundsChunkBudgetDownToBlockBoundary) {
  ScopedValue<bool> prefix_cache_guard(&FLAGS_enable_prefix_cache, true);
...
TEST(ChunkedPrefillSchedulerTest,
  ...
  int block_num = 32;
  int block_size = 128;
  int max_tokens_per_chunk_for_prefill = 128;
  ContinuousScheduler::Options opt =
  ...
  LinearStatePrefixCache* linear_state_prefix_cache =
      engine->block_manager_pool()->linear_state_prefix_cache(/*dp_rank=*/0);
  ASSERT_NE(linear_state_prefix_cache, nullptr);
  LinearStateCheckpointReservation checkpoint_reservation;
  const int32_t checkpoint_slot = linear_state_prefix_cache->reserve_checkpoint(
      checkpoint_hash, &checkpoint_reservation);
  ASSERT_GE(checkpoint_slot, 1);
  ASSERT_TRUE(checkpoint_reservation.valid());
  EXPECT_EQ(
      linear_state_prefix_cache->insert(std::move(checkpoint_reservation)),
      checkpoint_slot);
  engine->block_manager_pool()->deallocate_without_cache(cached_seq);

  scheduler->allocate_shared_blocks_for(active_seq);
  EXPECT_EQ(active_seq->kv_state().shared_kv_blocks_num(), 2u);
  EXPECT_EQ(active_seq->kv_state().num_kv_blocks(), 2u);
