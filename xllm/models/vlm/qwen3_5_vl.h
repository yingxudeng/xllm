...
using Qwen3_5_VLImageInputs = Qwen3_VLImageInputs;
using Qwen3_5_VLVideoInputs = Qwen3_VLVideoInputs;
using Qwen3_5_VLInputProcessor = Qwen3_VLInputProcessor;
using Qwen3_5_VLImageProcessor = Qwen3VLImageProcessor;
...
class Qwen3_5_VLForConditionalGenerationImpl : public torch::nn::Module {
 public:
  ...
  void prepare_encoder_input(
      ...
      std::optional<Qwen3_5_VLImageInputs>& image_inputs,
      std::optional<Qwen3_5_VLVideoInputs>& video_inputs) {
    ...
    if (pixel_values.defined() && image_grid_thw.defined()) {
      image_inputs = Qwen3_5_VLImageInputs{pixel_values, image_grid_thw};
    }

    if (pixel_values_videos.defined() && video_grid_thw.defined()) {
      video_inputs = Qwen3_5_VLVideoInputs{pixel_values_videos, video_grid_thw};
    }
  }

  MMDict get_multimodal_embeddings(const ModelInputParams& input_params) {
    std::optional<Qwen3_5_VLImageInputs> image_input;
    std::optional<Qwen3_5_VLVideoInputs> video_input;
    prepare_encoder_input(input_params, image_input, video_input);

    MMDict multimodal_embeds;
    const int32_t merge_size = model_args_.mm_spatial_merge_size();
    if (image_input) {
      torch::Tensor image_embeds =
          std::get<0>(visual_(image_input->pixel_values.to(options_),
    ...
    if (video_input) {
      torch::Tensor video_embeds =
          std::get<0>(visual_(video_input->pixel_values_videos.to(options_),
                              video_input->video_grid_thw.to(options_.device()),
                              input_params));

      torch::Tensor video_tokens =
  ...
  torch::Tensor get_input_embeddings(const torch::Tensor input_ids,
    ...
    torch::Tensor is_multimodal = torch::isin(input_ids, special_token_ids);
...
#define LOAD_QWEN3_5_VL_NESTED_ROPE_ARG(arg_name, json_key, default_value)  \
  LOAD_ARG_OR(arg_name, "text_config." #arg_name, default_value);           \
  LOAD_ARG_OR(arg_name, #arg_name, args->arg_name());                       \
  LOAD_ARG_OR(                                                              \
...
REGISTER_INPUT_PROCESSOR(qwen3_5_vl, Qwen3_5_VLInputProcessor);
REGISTER_CAUSAL_VLM_MODEL(qwen3_5_vl, Qwen3_5_VLForConditionalGeneration);
REGISTER_IMAGE_PROCESSOR(qwen3_5_vl, Qwen3_5_VLImageProcessor);
...
REGISTER_MODEL_ARGS(qwen3_5_vl, [&] {
  ...
  LOAD_QWEN3_5_VL_TEXT_OR_ROOT(eos_token_id, "eos_token_id", 248044);
  ...
  LOAD_QWEN3_5_VL_TEXT_OR_ROOT(hidden_size, "hidden_size", 4096);
  LOAD_QWEN3_5_VL_TEXT_OR_ROOT(initializer_range, "initializer_range", 0.02f);
  LOAD_QWEN3_5_VL_TEXT_OR_ROOT(intermediate_size, "intermediate_size", 12288);
  LOAD_QWEN3_5_VL_TEXT_OR_ROOT(
      max_position_embeddings, "max_position_embeddings", 262144);
  LOAD_QWEN3_5_VL_TEXT_OR_ROOT(max_window_layers, "max_window_layers", 28);
  LOAD_QWEN3_5_VL_TEXT_OR_ROOT(n_heads, "num_attention_heads", 16);
  LOAD_QWEN3_5_VL_TEXT_OR_ROOT(n_layers, "num_hidden_layers", 32);
  ...
  LOAD_QWEN3_5_VL_TEXT_OR_ROOT(
      linear_num_value_heads, "linear_num_value_heads", 32);
  LOAD_QWEN3_5_VL_TEXT_OR_ROOT(
  ...
  if (!args->mm_deepstack_visual_indexes().empty()) {
    LOG(FATAL) << "qwen3_5_vl does not support DeepStack visual indexes";
  }
  LOAD_ARG_OR(mm_temporal_patch_size, "vision_config.temporal_patch_size", 2);
  LOAD_ARG_OR_FUNC(mm_head_dim, "head_dim", [&] {
    return args->mm_hidden_size() / args->mm_num_attention_heads();
  });
  LOAD_QWEN3_5_VL_NESTED_ROPE_ARG(
      rope_scaling_rope_type, "rope_type", "default");
  LOAD_QWEN3_5_VL_NESTED_ROPE_ARG(rope_scaling_mrope_section,
                                  "mrope_section",
...
#undef LOAD_QWEN3_5_VL_NESTED_ROPE_ARG
