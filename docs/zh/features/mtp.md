...
!!! note "模型支持"
    ...
    - Qwen3.5（例如 [Qwen/Qwen3.5-9B](https://huggingface.co/Qwen/Qwen3.5-9B)，输入 model_type: qwen3_5 或 qwen3_5_text，导出 MTP model_type: qwen3_5_mtp）
    - MiMo（例如 [XiaomiMiMo/MiMo-7B-Base](https://huggingface.co/XiaomiMiMo/MiMo-7B-Base)，输入 model_type: mimo，导出 MTP model_type: mimo_mtp）
    ...
    - Qwen3.5 官方 checkpoint 内置原生 MTP 权重，使用 xLLM serving 前需要先导出为独立 draft model。
    - MiMo 官方 checkpoint 内置原生 MTP 权重（位于 `model.mtp_layers.*`），目前仅支持 CUDA 设备，使用 xLLM serving 前需要先导出为独立 draft model。
...
Qwen3.5 导出会从输入 checkpoint 读取原生的 `mtp.*` 或 `model.mtp.*` 权重，同时保留 draft model 需要的共享 embedding 和 `lm_head` 权重，并生成 `mtp_layer_parameters.safetensors` 以及 `model_type=qwen3_5_mtp` 的 MTP 配置。
...
MiMo 导出会从输入 checkpoint 读取 `model.mtp_layers.0.*` 权重，不做 key 重映射，直接以相同的 `model.mtp_layers.0.*` key 写入 `mtp_layer_parameters.safetensors`。导出的 config 将 `num_hidden_layers` 设置为 MTP 层数（1），与其他 MTP 模型保持一致。`embed_tokens` 和 `lm_head` 不包含在导出文件中，由运行时从 target 模型共享。
...
python3 tools/export_mtp.py \
    ...
    --model-type deepseek_v3  # 可选: deepseek_v3 (用于V3/R1), deepseek_v32 (用于V3.2), glm4_moe, mimo, qwen3_5, qwen3_5_moe
```
