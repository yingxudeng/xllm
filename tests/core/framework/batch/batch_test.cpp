...
TEST(BatchTest, LinearStateCheckpointSavesOnlyAtPrefillStepBoundary) {
  ...
  aligned_seq.add_kv_blocks(aligned_blocks);
  ...
  restore_seq.add_kv_blocks(restore_blocks);
  ...
  EXPECT_EQ(cache_ops[1].restore_prefix_hash, cache_ops[0].save_prefix_hash);
  EXPECT_FALSE(is_zero_prefix_hash(cache_ops[1].restore_prefix_hash));
  EXPECT_EQ(cache_ops[1].save_prefix_hash,
            compute_linear_state_prefix_hash_for_test(
                aligned_tokens, restore_blocks, /*boundary_tokens=*/20));
  EXPECT_TRUE(is_zero_prefix_hash(cache_ops[2].restore_prefix_hash));
  EXPECT_TRUE(is_zero_prefix_hash(cache_ops[2].save_prefix_hash));
  EXPECT_TRUE(is_zero_prefix_hash(cache_ops[3].restore_prefix_hash));
  EXPECT_TRUE(is_zero_prefix_hash(cache_ops[3].save_prefix_hash));
}
